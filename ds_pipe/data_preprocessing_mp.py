"""
Segundo step en el pipeline de DS. - Version Optimizada
Librería multiprocessing implementada en tools/optimizer.py
...
from tools.optimizer import many_funcs_one_arg
from utils.config import PARQUET_PATH


def update_legend(patterns: dict, ass_name_type: pd.Series) -> dict:
    """
    Applies the first regex operation to update legend dict.
    This function is defined to be executed in parallel.
    """
    result = {}
    for pattern in patterns:
        regex = re.compile(pattern)
        result[patterns[pattern]] = set(
            ass_name_type[ass_name_type.str.contains(regex)].tolist()
        )
    return result


def replace_patterns(patterns: dict, ass_name_type: pd.Series) -> pd.Series:
    """
    Applies the second regex operation to the data.
    This function is defined to be executed in parallel.
    """
    return ass_name_type.replace(patterns, regex=True)


def data_preprocessing_mp(parquet_name: Union[str, None] = None) -> None:
    """
    ...
    # Ejecucion de las funciones en procesos paralelos
    funcs = [update_legend, replace_patterns]
    args = patterns, ass_name_type
    results, ass_name_label = many_funcs_one_arg(funcs, *args)
    legend.update(results)
